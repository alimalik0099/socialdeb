...
<body>
      ...
      <div class="row">
         ...
         <div class="col-md-5">
            <div class="rounded p-3 border-1 mb-4"
               ...
               232, 240);">
            <?php
                   $query = "SELECT * FROM news WHERE category='$catid' ORDER BY id DESC";
                   $result = mysqli_query($conn, $query);  
                   if ($result->num_rows > 0) {
                    while($row = mysqli_fetch_array($result))  
                    {
   ...
   echo $content;
                     ...
                     <a href="single-news.php?news-id=<?php echo $row['id'];?>" class="btn btn-primary">Read More</a>
                  </div>
               </div>
               <?php 
           }
